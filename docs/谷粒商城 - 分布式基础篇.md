...
1. B2B(Business to Business)，是指商家与商家建立的商业关系。如：阿里巴巴。
2. B2C(Business to Consumer)，就是我们经常看到的供应商直接把商品卖给用户，即“商对客”模式，也就是通常说的商业零售，直接面向消费者销售产品和服务。如：苏宁易购、京东、天猫、小米商城。
3. C2B(Customer to Business)，即消费者对企业。先有消费者需求产生而后有企业生产，即先有消费者提出需求，后有生产企业按需求组织生产。
4. C2C(Customer to Consumer)，客户之间自己把东西放上网去卖，如:淘宝，闲鱼。
5. O2O 即 Online To Offline，将线下商务的机会与互联网结合在了一起，让互联网成为线下交易的前台。线上快速支付，线下优质服务。如:饿了么，美团，淘票票，京东到家。
...
![image-20210730000054148](.\images\谷粒商城-微服务划分图.png)
...
微服务架构风格，就像是把一个**单独的应用程序**开发为一套小服务，每个**小服务**运行在**自己**的**进程**中，并使用轻量级机制**通信**，通常是 HTTP API。这些服务困绕业务能力来构建，并通过完全**自动化部署**机制来独立部署。这些服务使用不同的编程语言书写，以及不同数据存储技术，并保持最低限度的集中式管理。
...
![image-20210730002330852](.\images\鼓励商城-远程调用.png)
...
![image-20210730002717563](.\images\谷粒商城-负载均衡.png)
...
<img src=".\images\谷粒商城-注册中心.png" alt="image-20210730002137030" style="zoom:150%;" />
...
![image-20210730003223868](.\images\谷粒商城-配置中心.png)
...
- 服各熔断
  ...
  设置服务的超时，当被调用的服务经常失败到达某个阈值，我们可以开
  启断路保护机制，后来的请求不再去调用这个服务。本地直接返回默认的数据

- 服各降级

  在运维期间，当系统处于高峰期，系统资源紧张，我们可以让非核心业
  务降级运行。降级:某些服务不处理，或者简单处理【抛异常、返回NULL、调用Mock数据、调用Fallback 处理逻辑】。

![image-20210730003849126](.\images\谷粒商城-服务熔断&降级.png)

## 8 API网关

在微服务架构中，API Gateway 作为整体架构的重要组件，它**抽象了微服务中都需要的公共功能**，同时提供了客户端**负载均衡，服务自动熔断，灰度发布，统一认证，限流流控，日志统计**等丰富的功能，帮助我们解决很多 API 管理难题。

![image-20210730004556090](.\images\谷粒商城-API网关.png)
